...
final class BHPlugin_Core {
 public static function init():void { add_action('init',[__CLASS__,'register_content']); add_action('admin_menu',[__CLASS__,'admin_menu']); add_action('wp_enqueue_scripts',[__CLASS__,'enqueue_frontend']); add_action('admin_enqueue_scripts',[__CLASS__,'enqueue_admin']); }
 ...
 public static function settings_page():void { if(!current_user_can('manage_options')) return; $available=array_filter([class_exists('ACF')?'ACF':'',class_exists('Ninja_Forms')?'Ninja Forms':'',class_exists('UM')?'Ultimate Member':'',class_exists('GetPaid')?'GetPaid':'']); echo '<div class="wrap"><h1>Bubba Hub</h1><p>Step 1: activity discovery foundation.</p><div class="notice notice-info inline"><p><strong>Integrations detected:</strong> '.esc_html(implode(', ',$available?:['None'])).'</p></div></div>'; }
 public static function enqueue_frontend():void { wp_enqueue_style('bhplugin-frontend',BHPLUGIN_URL.'assets/css/frontend.css',[],BHPLUGIN_VERSION); }
